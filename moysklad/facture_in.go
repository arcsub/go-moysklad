...
type FactureIn struct {
	...
	Name           *string              `json:"name,omitempty"`           // Наименование Счета-фактуры полученного
	AccountID      *uuid.UUID           `json:"accountId,omitempty"`      // ID учётной записи
	Code           *string              `json:"code,omitempty"`           // Код Счета-фактуры полученного
	...
	Description    *string              `json:"description,omitempty"`    // Комментарий Счета-фактуры полученного
	ExternalCode   *string              `json:"externalCode,omitempty"`   // Внешний код Счета-фактуры полученного
	Files          *MetaArray[File]     `json:"files,omitempty"`          // Метаданные массива Файлов (Максимальное количество файлов - 100)
	Group          *Group               `json:"group,omitempty"`          // Отдел сотрудника
	ID             *uuid.UUID           `json:"id,omitempty"`             // ID Счета-фактуры полученного
	Meta           *Meta                `json:"meta,omitempty"`           // Метаданные Счета-фактуры полученного
	...
	State          *NullValue[State]    `json:"state,omitempty"`          // Метаданные статуса Счета-фактуры полученного
	Sum            *float64             `json:"sum,omitempty"`            // Сумма Счета-фактуры полученного в установленной валюте
	SyncID         *uuid.UUID           `json:"syncId,omitempty"`         // ID синхронизации
	Updated        *Timestamp           `json:"updated,omitempty"`        // Момент последнего обновления Счета-фактуры полученного
...
// GetName возвращает Наименование Счета-фактуры полученного.
...
// GetCode возвращает Код Счета-фактуры полученного.
...
// GetDeleted возвращает Момент последнего удаления Счета-фактуры полученного.
func (factureIn FactureIn) GetDeleted() Timestamp {
	return Deref(factureIn.Deleted)
}

// GetDescription возвращает Комментарий Счета-фактуры полученного.
func (factureIn FactureIn) GetDescription() string {
	return Deref(factureIn.Description)
}

// GetExternalCode возвращает Внешний код Счета-фактуры полученного.
...
// GetID возвращает ID Счета-фактуры полученного.
func (factureIn FactureIn) GetID() uuid.UUID {
	return Deref(factureIn.ID)
}

// GetMeta возвращает Метаданные Счета-фактуры полученного.
...
// GetState возвращает Метаданные статуса Счета-фактуры полученного.
func (factureIn FactureIn) GetState() State {
	return factureIn.State.Get()
}

// GetSum возвращает Сумму Счета-фактуры полученного в установленной валюте.
...
// SetName устанавливает Наименование Счета-фактуры полученного.
func (factureIn *FactureIn) SetName(name string) *FactureIn {
	factureIn.Name = &name
	return factureIn
}

// SetCode устанавливает Код Счета-фактуры полученного.
...
// SetDescription устанавливает Комментарий Счета-фактуры полученного.
func (factureIn *FactureIn) SetDescription(description string) *FactureIn {
	factureIn.Description = &description
	return factureIn
}

// SetExternalCode устанавливает Внешний код Счета-фактуры полученного.
...
// SetMeta устанавливает Метаданные Счета-фактуры полученного.
...
type FactureInService interface {
	// GetList выполняет запрос на получение списка счетов-фактур полученных.
	// Принимает контекст и опционально объект параметров запроса Params.
	// Возвращает объект List.
	GetList(ctx context.Context, params ...*Params) (*List[FactureIn], *resty.Response, error)

	// Create выполняет запрос на создание счета-фактуры полученного.
	// Обязательные поля для заполнения:
	//	- incomingNumber (Входящий номер)
	//	- incomingDate (Входящая дата)
	// Принимает контекст, счет-фактуру полученный и опционально объект параметров запроса Params.
	// Возвращает созданную счет-фактуру полученный.
	Create(ctx context.Context, factureIn *FactureIn, params ...*Params) (*FactureIn, *resty.Response, error)

	// CreateUpdateMany выполняет запрос на массовое создание и/или изменение счетов-фактур полученных.
	// Изменяемые счета-фактуры полученные должны содержать идентификатор в виде метаданных.
	// Принимает контекст, список счетов-фактур полученных и опционально объект параметров запроса Params.
	// Возвращает список созданных и/или изменённых счетов-фактур полученных.
	CreateUpdateMany(ctx context.Context, factureInList Slice[FactureIn], params ...*Params) (*Slice[FactureIn], *resty.Response, error)

	// DeleteMany выполняет запрос на массовое удаление счетов-фактур полученных.
	// Принимает контекст и множество счетов-фактур полученных.
	// Возвращает объект DeleteManyResponse, содержащий информацию об успешном удалении или ошибку.
	DeleteMany(ctx context.Context, entities ...*FactureIn) (*DeleteManyResponse, *resty.Response, error)

	// Delete выполняет запрос на удаление счета-фактуры полученного.
	// Принимает контекст и ID счета-фактуры полученного.
	// Возвращает true в случае успешного удаления счета-фактуры полученного.
	Delete(ctx context.Context, id uuid.UUID) (bool, *resty.Response, error)

	// GetByID выполняет запрос на получение отдельного счета-фактуры полученного по ID.
	// Принимает контекст, ID счета-фактуры полученного и опционально объект параметров запроса Params.
	// Возвращает найденный счет-фактуру полученный.
	GetByID(ctx context.Context, id uuid.UUID, params ...*Params) (*FactureIn, *resty.Response, error)

	// Update выполняет запрос на изменение счета-фактуры полученного.
	// Принимает контекст, счет-фактуру полученный и опционально объект параметров запроса Params.
	// Возвращает изменённую счет-фактуру полученный.
	Update(ctx context.Context, id uuid.UUID, factureIn *FactureIn, params ...*Params) (*FactureIn, *resty.Response, error)

	// GetMetadata выполняет запрос на получение метаданных счетов-фактур полученных.
	// Принимает контекст.
	...
	// Template выполняет запрос на получение предзаполненного счета-фактуры полученного со стандартными полями.
	// без связи с какими-либо другими документами.
	// Принимает контекст.
	// Возвращает предзаполненный счет-фактуру полученный.
	Template(ctx context.Context) (*FactureIn, *resty.Response, error)

	// TemplateBased выполняет запрос на получение шаблона счета-фактуры полученного на основе других документов.
	...
	// Возвращает предзаполненный счет-фактуру полученный на основании переданных документов.
